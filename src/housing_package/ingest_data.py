...
def data_ingest(argv=None):
    ...
    parser.add_argument(
        "--path", help="path for the training dataset", type=str
    )
    parser.add_argument(
        "--log-level",
        help="logging level",
    ...
    housing_prepared.to_csv(
        os.path.join(TRAIN_DATA_PATH, "train.csv"), index=False
    )
    ...
    X_test_prepared.to_csv(
        os.path.join(TEST_DATA_PATH, "test.csv"), index=False
    )
    ...
    with mlflow.start_run(
        experiment_id=1, run_name="Ingest Data", nested=True
    ):
        ...
        mlflow.log_artifacts(
            os.path.join(os.getcwd(), "datasets", "housing"),
